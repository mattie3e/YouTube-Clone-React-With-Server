...
const WideNavComponent = (props) =>{
    ...
    return(
        <React.Fragment>
            <nav className={isOpen!=undefined ? (isOpen ? 'wide_nav open' : 'wide_nav close') : 'wide_nav'} onClick={setSelectedNav}>
                {
                    navMenuArr1.map((element, index) => {
                    return <WideNavItemComponent key={index} data={element} state={selectedNav}/>
                    })
                }
            ...
            <div className={isOpen!=undefined ? (isOpen ? 'block_box' : '') : ''}></div>
        </React.Fragment>
    )
